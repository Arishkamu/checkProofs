...
lemmaBindEmptyList k =
     [] >>= k          --. L (Inst ">>=")
 === concat (map k []) --. L (Decl "map")
 === concat []         --. L (Decl "concat")
 === foldr (++) [] []  --. L (Decl "foldr")
 === []                --. QED
...
lemmaBindNonEmptyList (x:xs) k =
     x : xs >>= k                    --. L (Inst ">>=")
 === concat (map k (x:xs))           --. L (Decl "map")
 === concat (k x : map k xs)         --. L (Decl "concat")
 === foldr (++) [] (k x : map k xs)  --. L (Decl "foldr")
 === k x ++ foldr (++) [] (map k xs) --. L (Decl "concat")
...
monad1LawList a k =
     return a >>= k           --. L (Inst "return")
 === a : [] >>= k             --. L (Inst ">>=")
 === concat (map k (a : []))  --. L (Decl "map")
 === concat (k a : map k [])  --. L (Decl "map")
 === concat (k a : [])        --. L (Decl "concat")
 === foldr (++) [] (k a : []) --. L (Decl "foldr")
 === k a ++ foldr (++) [] []  --. L (Decl "foldr")
